...
Original file is located at
    https://colab.research.google.com/drive/1R0o0qSMjiXsKY7ZBVCO97RRrpOmQDV_N
"""
...
with DAG(
    ...
    schedule_interval=None,  # Runs only when triggered manually
    catchup=False
) as dag:
    ...
    def create_table():
        ...
        create_table_query = """
        CREATE OR REPLACE TABLE data226.stock_price_real_time(
    ...
    def insert_data(data):
        """Insert data into Snowflake table with idempotency."""
        conn = get_snowflake_conn()
        cur = conn.cursor()
        try:
            # Fetch existing dates from the table
            existing_dates_query = "SELECT date FROM data226.stock_price_real_time"
            cur.execute(existing_dates_query)
            existing_dates = {row[0] for row in cur.fetchall()}

            insert_query = """
            INSERT INTO data226.stock_price_real_time (date, open, high, low, close, volume, timestamp)
    ...
    stock_prices = real_time_data('MSFT')
